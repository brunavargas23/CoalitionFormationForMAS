...
//+default::job(_, _, _, _, _, _) : not initiator::accept_jobs <- !update_free; !update_received_job;.
//+default::job(_, _, _, _, _, _) : action::hold_action(Id) <- .print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% I was analysing Job ",Id," ignoring new job."); !update_eval; !update_received_job;.
//@job[atomic]
...
//	!evaluate_job(Items, End, Storage, Id, Reward, fast);	
//	.
//+default::job(Id, Storage, Reward, Start, End, Items) : default::check_bases(Items,Result) & Result == "true" <- !update_received_job;.
//+default::job(Id, Storage, Reward, Start, End, Items) <- .print("Ignoring job, free agents ",Id); !update_free; !update_received_job; .

+default::job(Id, Storage, Reward, Start, End, Items)
<-
//	.print("New job ",Id," deliver to ",Storage," for ",Reward," starting at ",Start," to ",End);
	!update_received_job;
	!update_interval_job(Start);
	!check_feasible_job(Id, Storage, Reward, Start, End, Items);
	.
+!check_feasible_job(_,_,_,_,_,_)
	: not initiator::accept_jobs 
<- 
	!update_free;
	.
+!check_feasible_job(Id,_,_,_,_,_)
	: action::hold_action(Id) 
<- 
	.print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% I was analysing Job ",Id," ignoring new job."); 
	!update_eval;
	.
@job[atomic]
+!check_feasible_job(Id, Storage, Reward, Start, End, Items) 
...
<- 
	...
	!evaluate_job(Items, End, Storage, Id, Reward, fast);	
	.
+!check_feasible_job(Id,_,_,_,_,_) <- .print("Ignoring job, free agents ",Id); !update_free; .
...
+!evaluate_job(Items, End, Storage, Id, Reward, Characteristic)
...
//	if ( default::check_tools(ListToolsNew,AvailableTools,ResultT) & ResultT == "true" & default::check_buy_list(ListItemsConcat,ResultB) & ResultB == "true" & default::check_multiple_buy(ListItemsConcat,AddSteps) & default::check_price(ListToolsNew,ListItems,0,ResultP) & .print("Estimated cost ",ResultP * 1.1," reward ",Reward) & ResultP * 1.1 < Reward & actions.closest(Role,WList,Storage,ClosestWorkshop) & actions.route(Role,Speed,FarthestShop,ClosestWorkshop,RouteWorkshop) & actions.route(Role,Speed,ClosestWorkshop,Storage,RouteStorage) & Estimate = RouteShop+RouteWorkshop+RouteStorage+NumberOfBuyTool+NumberOfBuyItem+NumberOfAssemble+AddSteps+25 & .print("Estimate ",Estimate+Step," < ",End) & Estimate + Step < End & Step + Estimate < TotalSteps ) {
	if ( default::check_multiple_buy(ListItemsConcat,AddSteps) & default::check_price(ListToolsNew,ListItems,0,ResultP) & .print("Estimated cost ",ResultP * 1.1," reward ",Reward) & ResultP * 1.1 < Reward & actions.closest(Role,WList,Storage,ClosestWorkshop) & actions.route(Role,Speed,FarthestShop,ClosestWorkshop,RouteWorkshop) & actions.route(Role,Speed,ClosestWorkshop,Storage,RouteStorage) & Estimate = RouteShop+RouteWorkshop+RouteStorage+NumberOfBuyTool+NumberOfBuyItem+NumberOfAssemble+AddSteps+25 & .print("Estimate ",Estimate+Step," < ",End) & Estimate + Step < End & Step + Estimate < TotalSteps ) {
//		!!separate_tasks(Id, Storage, ListItems, ListToolsNew, Items);
		
		+::job(Id,ListItems,ListToolsNew,Items,End,Storage,Reward);
		!run_just_task_allocation;
//		!run_coalition_algorithm;
	}
	else { 
...
<-
	...
	if ( Step + 40 < TotalSteps & Step + 40 < End ) {
		...
		!decompose(Items,ListItems,ListToolsNew,Id);
//		!!separate_tasks(Id, Storage, ListItems, ListToolsNew, Items);
		!run_coalition_algorithm(Id, Storage, ListItems, ListToolsNew, Items, Reward, slow);
	}
	else { 
...
@generateCoalitionId[atomic]
+!generate_coalition_id(NewId)
	: ::artifactCoalitionIdCount(Id)
<-
	-+::artifactCoalitionIdCount(Id+1);
	.concat("cf_",Id+1,StrId);
	.term2string(NewId,StrId);
	.
+!run_coalition_algorithm
	: .findall(job(Id,ListItems,ListTools,Storage,Reward),::job(Id,ListItems,ListTools,Items,End,Storage,Reward),ListJobs) & (.length(ListJobs) >= 3) & initiator::free_agents(FreeAgents)
<-
	.print("Initiator will run coalition algorithm to ",ListJobs);
	.print("Free agents: ",FreeAgents);
//	-initiator::accept_jobs;
	!generate_coalition_id(NewId);
	
	!coalition::setup_coalition_artefact(NewId,.length(FreeAgents),.length(ListJobs),ListJobs);
	.send(FreeAgents,achieve,coalition::form_coalition(FreeAgents,NewId,ListJobs));
	.
+!run_coalition_algorithm
<-
...
+!run_just_task_allocation
	: .findall(job(Id,ListItems,ListTools,Items,Storage),::job(Id,ListItems,ListTools,Items,End,Storage,Reward),ListJobs) & (.length(ListJobs) >= 3)
<-
	for(.member(job(Id,_,_,_,_),ListJobs)){
		+action::hold_action(Id);
	}
	for(.member(job(Id,ListItems,ListToolsNew,Items,Storage),ListJobs)){
		!!separate_tasks(Id, Storage, ListItems, ListToolsNew, Items);
	}
	.
+!run_just_task_allocation
...
//	: not cnp(_) & new::max_bid_time(Deadline) & initiator::coalitionTrucks(Id,FreeTrucks) & .length(FreeTrucks,NumberOfTrucks) & initiator::coalitionAgents(Id,FreeAgents) & .length(FreeAgents,NumberOfAgents) & NumberOfTrucks > 0 
<-
	+cnp(Id);
	+job(Id, Items);
...
//	.print("Received all bids ",JobId);
	-::coalitionAgents(JobId,_);
	-::coalitionTrucks(JobId,_);
	-::job(JobId,_,_,_,_,_,_);
...
//				.print("Awarding assemble of ",JobId," to ",Agent);
				...
				getLoad(Agent,Load);
				?default::total_load(Items,0,Volume);
				addLoad(Agent,Load-Volume);
				+awarded_assemble(Agent,Items,StorageId,[],JobId,0);
			}
...
+!add_agent_to_free[source(Agent)] 
	: initiator::free_agents(FreeAgents) & not .member(Agent,FreeAgents)
<-
...
+!add_truck_to_free[source(Agent)]
	: initiator::free_agents(FreeAgents) & initiator::free_trucks(FreeTrucks) & not .member(Agent,FreeAgents) & not .member(Agent,FreeTrucks)
<-
	-+initiator::free_agents([Agent|FreeAgents]);
...
@addMeFree[atomic]
+!add_myself_to_free
	: initiator::free_agents(FreeAgents) & .my_name(Me) & not .member(Me,FreeAgents)
<-
...
@upateCoalition[atomic]
+!update_coalition_failed
	: metrics::failedCoalition(C)
<-
	-+metrics::failedCoalition(C+1);
.

@upateTaskAllocation[atomic]
+!update_ta_failed
	: metrics::failedTA(C)
<-
	.print("TA failed: ",C+1);
	-+metrics::failedTA(C+1);
.
...
@upateIntervalJob[atomic]
+!update_interval_job(StartJob)
	: ::intervalBetweenJobs(I)
<-
//	.print("It took ",StartJob-I," to get a new job");
	actions.recordStatistics(jobInterval,StartJob-I);
	-+::intervalBetweenJobs(StartJob);
	.
@upateReceivedJob[atomic]
+!update_received_job
	: metrics::receivedJobs(C)
<-
	-+metrics::receivedJobs(C+1);
.
