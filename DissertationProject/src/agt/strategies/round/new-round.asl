...
<-
	...
	+initiator::artifactCoalitionIdCount(0);
	+initiator::intervalBetweenJobs(0);
	...
	+metrics::failedCoalition(0);
	+metrics::failedTA(0);
	+metrics::receivedJobs(0);
...
@recordStepMoney[atomic]
+!recordStatistics(step)
	: .my_name(vehicle1) & default::step(Step) & default::money(Money)
<-
	actions.recordStatistics(step,Step,Money);
	.
+!recordStatistics(step).
...
//	+tool_types([vehicle1,vehicle5,vehicle13,vehicle21]);
	+tool_types([vehicle1,vehicle4,vehicle7,vehicle10]);
	+vehicle_job(truck,2);
	+noActionCount(0);
...
//	+::listFreeTrucks([vehicle20,vehicle21,vehicle22,vehicle23,vehicle24,vehicle25,vehicle26,vehicle27,vehicle28]);
	+::listFreeTrucks([vehicle10,vehicle11,vehicle12]);
