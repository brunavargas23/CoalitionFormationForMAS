...
<-
	...
	?metrics::failedCoalition(CoalitionFailed); // Ok
	.print("There was no coalition structure: ",CoalitionFailed);
	?metrics::failedTA(TAFailed); // Ok
	.print("Task allocation failed: ",TAFailed);
	?metrics::receivedJobs(ReceivedJobs); // Ok
	.print("Received Jobs: ",ReceivedJobs);
	
	?metrics::numberOfShops(NShops);
	?metrics::numberOfWorkshops(NWorkshops);
	?metrics::numberOfStorages(NStorages);
	
	actions.recordStatistics(endRoundTeam,Money,ReceivedJobs,JobsCompleted,JobsFailed,JobsFreeFailed,JobsEvalFailed,CoalitionFailed,TAFailed,NShops,NWorkshops,NStorages);
...
<-
	...
	?metrics::moneyEarned(MoneyEarned);  // Ok
	.print("Money I have earned: ",MoneyEarned);
	
	actions.recordStatistics(endRoundIndividual,Me,Role,NoActions,Jobs,JobsFail,MoneyEarned);
	.
